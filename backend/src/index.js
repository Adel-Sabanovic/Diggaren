...
function startingOut(){ //potentiellt lägga till in letiabler artist/track/album här som sedan skickas med till searchSpotify, så att delen från sveriges radio bara anropar denna
  if(token==null){
   getNewToken();
   searchSpotify("sunburn", "muse", "showbiz")
  }
  else if(Date.now > expires){
   getNewToken();
  }
  else{
  searchSpotify("sunburn", "muse", "showbiz") //potentiellt invariabler här
  }
}
